...
int calcDenseWarpFlowGPU(char *file_name, char *out_file_name, int bound, int type, int step, int dev_id, int new_width, int new_height){
	...
	while(true){
		...
		if (!initialized){
			...
			if (!do_resize){
				...
				human_mask = Mat::ones(new_size, CV_8UC1);
			}
	...
	std::string xFlowFile = string(out_file_name) + "flow_x";
	std::string yFlowFile = string(out_file_name) + "flow_y";
	std::string imgFile = string(out_file_name) + "flow_i";
...
int main()
{
	...
	calcDenseWarpFlowGPU("E://好好//test_好.avi", out_path, 15, 0, 1,0,10,10);
	return 0;
}
